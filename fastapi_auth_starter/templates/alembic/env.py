...
if not config.get_main_option("sqlalchemy.url"):
    try:
        ...
        # Settings not configured yet - provide helpful error message
        import sys
        print(
            "\n❌ Error: DATABASE_URL not found. Please configure your .env file first:\n"
            "   1. Copy .env.example to .env: cp .env.example .env\n"
            "   2. Edit .env and set DATABASE_URL with your database connection string\n"
            "   3. Then run this command again\n",
            file=sys.stderr
        )
        raise
